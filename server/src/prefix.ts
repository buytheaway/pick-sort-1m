...
export const buildPrefixRanges = (prefix: string, maxValue: number) => {
  if (!prefix) return [{ start: 1, end: maxValue }];
  if (prefix.startsWith("0")) return [];
  ...
  for (let extra = 0; extra <= maxLen - prefixLen; extra += 1) {
    ...
    if (end < 1 || start > maxValue) continue;
    if (start < 1) start = 1;
...
export const initRangeCursor = (
  ...
  for (let i = 0; i < ranges.length; i += 1) {
    ...
    if (!range) continue;
    if (afterId < range.start) return { rangeIndex: i, value: range.start };
...
export const advanceRangeCursor = (
  ranges: PrefixRange[],
  cursor: RangeCursor
): RangeCursor | null => {
  ...
  while (rangeIndex < ranges.length) {
    ...
    if (value <= range.end) return { rangeIndex, value };
    rangeIndex += 1;
