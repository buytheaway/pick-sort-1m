...
const syncLogger = (req, _res, next) => {
    ...
    if (Array.isArray(ops.addIds))
        parts.push(`addIds(${ops.addIds.length})`);
    ...
    if (views.left)
        respParts.push("left");
    if (views.right)
        respParts.push("right");
    const respText = respParts.length ? respParts.join(",") : "none";
    const stamp = new Date().toISOString();
    console.log(`[${stamp}] /api/sync views=${hasViews ? 1 : 0} ops=${opsText} resp=${respText}`);
    next();
};
