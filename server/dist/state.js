import { advanceRangeCursor, buildPrefixRanges, initRangeCursor, sanitizeFilter, } from "./prefix.js";
const LIMIT = 20;
...
const matchesPrefix = (id, prefix) => {
    if (!prefix)
        return true;
    return String(id).startsWith(prefix);
};
const normalizeId = (value) => Number.isFinite(value) ? Math.trunc(value) : NaN;
...
const insertSortedUnique = (arr, value) => {
    let lo = 0;
    let hi = arr.length;
    while (lo < hi) {
        const mid = (lo + hi) >> 1;
...
const getNextAdded = (startIndex, filter, afterId) => {
...
const getNextBase = (filter, afterId) => {
...
export const applyOps = (ops) => {
    ...
    if (Array.isArray(ops.selectRemove)) {
        for (const raw of ops.selectRemove) {
            ...
            if (!Number.isFinite(id) || id <= 0) {
                ...
                rejected.selectRemove.push(raw);
                continue;
            }
            if (!state.selectedSet.has(id)) {
                rejected.selectRemove ??= [];
        ...
        if (changed) {
            state.selectedOrder = state.selectedOrder.filter((id) => state.selectedSet.has(id));
        }
    }
    if (ops.reorder && Number.isFinite(ops.reorder.dragId)) {
        ...
        if (Number.isFinite(dragId) && state.selectedSet.has(dragId)) {
            ...
            const filtered = state.selectedOrder.filter((id) => matchesPrefix(id, filter));
            const currentIndex = filtered.indexOf(dragId);
            if (currentIndex !== -1 && filtered.length > 0) {
                const targetIndex = Math.max(0, Math.min(normalizeNonNegative(ops.reorder.toIndex), filtered.length - 1));
...
export const getRightView = (views) => {
...
export const getLeftView = (views) => {
    const filter = sanitizeFilter(views?.filter);
